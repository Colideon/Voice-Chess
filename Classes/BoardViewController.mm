...
- (void)loadView {
   ...
   if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
      ...
      [buttons addObject: button];*/
       ...
       barShowHidePiece = [[UIBarButtonItem alloc] initWithTitle: @"Hide"
                                                           style: UIBarButtonItemStyleBordered
                                                          target: self
                                                          action: @selector(toolbarButtonPressed:)];
       [barShowHidePiece setWidth: 49.0f];
       [buttons addObject: barShowHidePiece];
   ...
   gameMenu = [[UIActionSheet alloc]
                ...
                otherButtonTitles: @"New game", @"Save game", @"Load game", @"E-mail game", @"Edit position", @"Level/Game mode", @"Support us", nil];
...
- (void)doDoubleTap{
    NSLog(@"###########    Double tap");
    [gameController startListeningTo];
}
...
clickedButtonAtIndex:(NSInteger)buttonIndex {
   ...
   if (actionSheet == gameMenu) {
      ...
      else if ([buttonTitle isEqualToString: @"Support us"])
          dispatch_async(dispatch_get_main_queue(), ^{
              [[UIApplication sharedApplication] openURL:[NSURL URLWithString: @"http://colideon.com/Voice_chess/"]];
          });
   }
...
- (void)toolbarButtonPressed:(id)sender {
   ...
   else if ([title isEqualToString: @"Show"])
   {
       [gameController showHidePieces:YES];
       barShowHidePiece.title = @"Hide";
   }
   else if ([title isEqualToString: @"Hide"])
   {
       [gameController showHidePieces:NO];
       barShowHidePiece.title = @"Show";
   }
