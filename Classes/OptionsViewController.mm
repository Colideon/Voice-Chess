   ...
   if (section == 0) {
      ...
      forControlEvents:UIControlEventValueChanged];
         ...
         [[cell detailTextLabel] setText: @""];
          ...
          [[cell textLabel] setText: @"Audio support"];
          sw = [[UISwitch alloc] initWithFrame: CGRectMake(4.0f, 16.0f, 10.0f, 28.0f)];
          [sw setOn: [Options sharedOptions].audioSupport animated: NO];
          [sw addTarget: self action: @selector(toggleAudioSupport:)
       forControlEvents:UIControlEventValueChanged];
          [cell setAccessoryView: sw];
          [[cell detailTextLabel] setText: @""];
      }
...
// New
- (void)toggleAudioSupport:(id)sender {
    [Options sharedOptions].audioSupport = [sender isOn];
}
