...
- (id)initWithBoardView:(BoardView *)bv
   ...
   if (self = [super init]) {
      ...
      for (int i = 0; i < 8; i++) {
       ...
       NSURL *beepSoundURL = [[NSBundle mainBundle] URLForResource: @"vcBeep3"
                                                     withExtension: @"wav"];
       AudioServicesCreateSystemSoundID((__bridge CFURLRef)beepSoundURL, &vcBeepSound);
...
- (void) prepareSpeech
{
    ...
    // New params
    speechService.cancelWhenSilent = YES;
//    speechService.maxInitialSilence = ATTSpeechServiceInfiniteInterval;
    speechService.maxInitialSilence = 2.0f;
//    speechService.endingSilence = 2.7f;
...
{
    // Added
    if ([Options sharedOptions].audioSupport == false) return;
    
    // Play beep sound.
    AudioServicesPlaySystemSound(vcBeepSound);
    
    [self startListeningAfter:0.7f];
}
...
- (void) speechServiceSucceeded: (ATTSpeechService*) speechService
{
    ...
    NSLog(@"You told: %@", recognizedText);
//    [self processResponse: parsedResponse];
    
    // Added
    [self makeMoveFromSentence:recognizedText];
}
...
{
    if ([error.domain isEqualToString: ATTSpeechServiceErrorDomain] && (error.code == ATTSpeechServiceErrorCodeCanceledByUser)) {
        NSLog(@"Speech service canceled");
        // Nothing to do in this case
        return;
    }
    else if ([error.domain isEqualToString: ATTSpeechServiceErrorDomain] && (error.code == ATTSpeechServiceErrorCodeNoMicrophone)) {
        NSLog(@"Speech service no mic");
        [self OESaySentence:@"No, microphone"];
        // Nothing to do in this case
        return;
    }
    else if ([error.domain isEqualToString: ATTSpeechServiceErrorDomain] && (error.code == ATTSpeechServiceErrorCodeNoAudio)) {
        NSLog(@"Speech service no audio");
    //    [self OESaySentence:@"No, audio"];
        [self playBeepAfter:1.0f];
        // Nothing to do in this case
        return;
    }
...
- (void) startListeningAfter: (float) sec
{
    [self performSelector:@selector(startingListening)
               withObject:nil
               afterDelay:sec];
}

- (void) repeatListening: (float) sec
{
    [self performSelector:@selector(startListeningTo)
               withObject:nil
               afterDelay:sec];
}

- (void) playBeepAfter: (float) sec
{
    [self performSelector:@selector(playBeepSound)
               withObject:nil
               afterDelay:sec];
}
...
- (void)initOpenEars {
    ...
    chessNumbers = @[@"one", @"two", @"three", @"four", @"five", @"six", @"seven", @"eight"];
    chessLetters = @[@"alfa", @"bravo", @"charlie", @"delta", @"echo", @"foxtrot", @"golf", @"hotel"];
    chessPieces = @[@"PAWN", @"KNIGHT", @"BISHOP", @"ROOK", @"QUEEN", @"KING"];

    chessListens = @[@"alfa", @"bravo", @"charlie", @"delta", @"echo", @"foxtrot", @"golf", @"hotel",
                     @"one", @"two", @"three", @"four", @"five", @"six", @"seven", @"eight"];

}

// Say string with OpenEars
- (void)OESaySentence:(NSString *)hypothesis {
    [self.fliteController say:[NSString stringWithFormat:@"%@",hypothesis] withVoice:self.slt];
    
//    [self repeatListening:3.0];
}

// Make sentence (string) from piece type and two squares
- (NSString *)sentenceFromSquare:(PieceType)pieceType from:(Square)from to:(Square)to {
    NSString *str = [NSString stringWithFormat:@" %@,  %@,  %@, TO,  %@,  %@",
                     chessPieces[pieceType - 1],
                     chessLetters[from % 8], chessNumbers[from / 8],
                     chessLetters[to % 8], chessNumbers[to / 8]];
    return str;
}

// Make chess move from sentence (string)
- (void)makeMoveFromSentence:(NSString *)hypothesis {
    NSArray *myWords = [hypothesis componentsSeparatedByString:@" "];
    
    
    if (wantToPromote == YES) {
        
        if ([myWords count] == 1) {
            NSLog(@"Trying to promote!!!!!!!!!!!");
            // else if want to promote and returned one word, try promote
            NSString *numberOne = myWords[0];
            NSUInteger numberOneNo = [chessListens indexOfObject:numberOne];
        
            if (numberOneNo >= 8 && numberOneNo < 12) {
                wantToPromote = NO;
                [promoteAC dismissWithClickedButtonIndex:-1 animated:YES];
                [self closeActionSheet:promoteAC clickedButtonAtIndex:numberOneNo - 8];
            } else {
                [self.fliteController say:[NSString stringWithFormat:@"Cannot promote"] withVoice:self.slt];
                [self repeatListening:1.8];
            }
        }
        else {
            [self.fliteController say:[NSString stringWithFormat:@"Wrong action"] withVoice:self.slt];
            [self repeatListening:1.8];
        }
    }
    else {
        ...
        if ([myWords count] == 4) {
            ...
            // If hypothesis got four words, try to move piece.
            NSString *letterOne = myWords[0];
            NSString *numberOne = myWords[1];
            NSString *letterTwo = myWords[2];
            NSString *numberTwo = myWords[3];
        
        
            NSUInteger letterOneNo = [chessListens indexOfObject:letterOne];
            NSUInteger numberOneNo = [chessListens indexOfObject:numberOne];
            NSUInteger letterTwoNo = [chessListens indexOfObject:letterTwo];
            NSUInteger numberTwoNo = [chessListens indexOfObject:numberTwo];
        
            if (letterOneNo < 8 &&
                numberOneNo >= 8 && numberOneNo < 16 &&
                letterTwoNo < 8 &&
                numberTwoNo >= 8 && numberTwoNo < 16) {
            
                Square squareFrom = Square((numberOneNo - 8) * 8 + letterOneNo);
                Square squareTo = Square((numberTwoNo - 8) * 8 + letterTwoNo);
            
            
                if (rotated) {
                    squareFrom = [self rotateSquare:squareFrom];
                    squareTo = [self rotateSquare:squareTo];
                }
            
                if ([self pieceCanMoveFrom: squareFrom to: squareTo]) {
                    [boardView stopHighlighting];
                
                    // Make the piece slide smoothly to the center of the square:
                    [self animateMoveFrom:squareFrom to:squareTo];

                } else {
                    [self.fliteController say:[NSString stringWithFormat:@"Cannot move"] withVoice:self.slt];
                //    [self repeatListening:1.8];
                }
            
            } else {
                [self.fliteController say:[NSString stringWithFormat:@"Wrong squares"] withVoice:self.slt];
            //    [self repeatListening:1.8];
            }
        } else {
            // else no four words
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    // New
    [self closeActionSheet:actionSheet clickedButtonAtIndex:buttonIndex];
}

// New my action sheet close with button inder
- (void)closeActionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {

    wantToPromote = NO;
    
    if ([[actionSheet title] isEqualToString: @"Promote to"]) {
        
        NSLog(@"Closing action sheet, Promote to");
...
- (void)doMoveFrom:(Square)fSq to:(Square)tSq promotion:(PieceType)prom {
   ...
   if ([game pieceCanMoveFrom: fSq to: tSq] > 1 && prom == NO_PIECE_TYPE) {
      ...
      pendingTo = [self rotateSquare: tSq];
       ...
       [promoteAC showInView: [boardView superview]];
       
       
       // New
       if ([Options sharedOptions].audioSupport) {
           wantToPromote = YES;
           [self.fliteController say:[NSString stringWithFormat:@"Promote to, Queen one, Rook two, Knight three, Bishop four"] withVoice:self.slt];
           
           [self repeatListening:4.5];
       }
...
- (void)doEngineMove:(Move)m {
   ...
   [game doMove: m];
    ...
    // New
    if ([Options sharedOptions].audioSupport) {
        Square newFrom, newTo;
        make_squares_from_move(m, newFrom, newTo);
        // Castle?
        if (move_is_long_castle(m)) [self OESaySentence:@"long, castle"];
        else if (move_is_short_castle(m)) [self OESaySentence:@"short, castle"];
        else
        {
            PieceType pieceType = [game typeOfPiece: newTo];
            [self OESaySentence:[self sentenceFromSquare:pieceType from:newFrom to:newTo]];
        }
    }
...
- (void)gameEndTest {
   if ([game positionIsMate]) {
      ...
      [game setResult: (([game sideToMove] == WHITE)? @"0-1" : @"1-0")];
       ...
       // New
       NSString *whoWins = ([game sideToMove] == WHITE)? @"Black, wins" : @"White, wins";
       [self.fliteController say:[NSString stringWithFormat:@"%@", whoWins] withVoice:self.slt];
      ...
      [[[UIAlertView alloc] initWithTitle: @"Game drawn"
       ...
       // New
       [self.fliteController say:[NSString stringWithFormat:@"Game, drawn"] withVoice:self.slt];
   }
}
