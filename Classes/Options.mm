...
@dynamic showAnalysis, showArrow, audioSupport, showBookMoves, showLegalMoves, permanentBrain, showCoordinates;
...
// New
#define KEY_AUDIO_SUPPORT @"audioSupport2"
...
- (id)init {
   if (self = [super init]) {
      ...
      if (![defaults objectForKey:KEY_SHOW_ARROW]) {
       ...
       // New
       if (![defaults objectForKey:KEY_AUDIO_SUPPORT]) {
           audioSupport = YES;
           [defaults setBool: YES forKey:KEY_AUDIO_SUPPORT];
       } else {
           audioSupport = [defaults boolForKey:KEY_AUDIO_SUPPORT];
       }
...
- (void)setAudioSupport:(BOOL)shouldAudioSupport {
    audioSupport = shouldAudioSupport;
    [[NSUserDefaults standardUserDefaults] setBool:audioSupport
                                            forKey:KEY_AUDIO_SUPPORT];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
